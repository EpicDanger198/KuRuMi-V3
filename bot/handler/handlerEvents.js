...
async function trackCommandSpam(threadID, threadName, globalData, message) {
        const config = global.KuRuMiV3.config;
...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
	return async function (event, message) {
		...
		const { utils, client, KuRuMiV3 } = global;
		const { getPrefix, removeHomeDir, log, getTime } = utils;
		const { config, configCommands: { envGlobal, envCommands, envEvents } } = KuRuMiV3;
		...
		*/
		...
		async function onStart() {
			...
			let command = KuRuMiV3.commands.get(commandName) || KuRuMiV3.commands.get(KuRuMiV3.aliases.get(commandName));
			// ———————— CHECK ALIASES SET BY GROUP ———————— //
			const aliasesData = threadData.data.aliases || {};
			for (const cmdName in aliasesData) {
				if (aliasesData[cmdName].includes(commandName)) {
					command = KuRuMiV3.commands.get(cmdName);
					break;
				}
			}
			// ————————————— SET COMMAND NAME ————————————— //
	...
	return true;
		...
		*/
		async function onChat() {
			const allOnChat = KuRuMiV3.onChat || [];
			const args = body ? body.split(/ +/) : [];
			for (const key of allOnChat) {
				const command = KuRuMiV3.commands.get(key);
				if (!command)
					continue;
				const commandName = command.config.name;
		...
		*/
		async function onAnyEvent() {
			const allOnAnyEvent = KuRuMiV3.onAnyEvent || [];
			let args = [];
		...
		*/
		async function onFirstChat() {
			const allOnFirstChat = KuRuMiV3.onFirstChat || [];
			const args = body ? body.split(/ +/) : [];
			...
			for (const itemOnFirstChat of allOnFirstChat) {
				...
				if (threadIDsChattedFirstTime.includes(threadID))
					continue;
				const command = KuRuMiV3.commands.get(commandName);
				if (!command)
					continue;
		...
		*/
		async function handlerEvent() {
			const { author } = event;
			const allEventCommand = KuRuMiV3.eventCommands.entries();
			for (const [key] of allEventCommand) {
				const getEvent = KuRuMiV3.eventCommands.get(key);
				if (!getEvent)
					continue;
		...
		*/
		async function onEvent() {
			const allOnEvent = KuRuMiV3.onEvent || [];
			const args = [];
			const { author } = event;
