...
module.exports = (
  ...
  return async function (event) {
    try {
      ...
      // ————————— CHECK ANTI INBOX —————————
      if (
        global.KuRuMiV3.config?.antiInbox === true &&
      ...
      // ————————— APPROVAL SYSTEM —————————
      if (global.KuRuMiV3.config?.approval) {
        let approvedtid =
      ...
      switch (event.type) {
        ...
        case "message_reaction":
          ...
          const { delete: del = [], kick = [] } =
            global.KuRuMiV3.config?.reactBy || {};

          // ——— DELETE MESSAGE REACTION ———
          if (del.includes(event.reaction)) {
            if (event.senderID === api.getCurrentUserID()) {
              if (global.KuRuMiV3.config?.adminBot?.includes(event.userID)) {
