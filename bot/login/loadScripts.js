...
// KuRuMiV3 moved inside function
const { configCommands } = KuRuMiV3;
		...
		for (const data of aliasesData) {
			...
			for (const alias of aliases)
				if (KuRuMiV3.aliases.has(alias))
					...
					KuRuMiV3.aliases.set(alias, commandName);
		}
	}
	...
	for (const folderModules of folders) {
		...
		for (const file of Files) {
			...
			try {
				...
				if (KuRuMiV3[setMap].has(commandName))
					throw new Error(`${text} "${commandName}" already exists with file "${removeHomeDir(KuRuMiV3[setMap].get(commandName).location || "")}"`);
				...
				if (aliases) {
					...
					for (const alias of aliases) {
						...
						if (KuRuMiV3.aliases.has(alias))
							throw new Error(`alias "${alias}" already exists in ${text} "${KuRuMiV3.aliases.get(alias)}" with file "${removeHomeDir(KuRuMiV3[setMap].get(KuRuMiV3.aliases.get(alias))?.location || "")}"`);
				...
				// ——————————————— CHECK RUN ANYTIME ——————————————— //
				if (onChat)
					KuRuMiV3.onChat.push(commandName);

				// ——————————————— CHECK ONFIRSTCHAT ——————————————— //
				if (onFirstChat)
					KuRuMiV3.onFirstChat.push({ commandName, threadIDsChattedFirstTime: [] });

				// ————————————————— CHECK ONEVENT ————————————————— //
				if (onEvent)
					KuRuMiV3.onEvent.push(commandName);

				// ———————————————— CHECK ONANYEVENT ———————————————— //
				if (onAnyEvent)
					KuRuMiV3.onAnyEvent.push(commandName);

				// —————————————— IMPORT TO GLOBALGOAT —————————————— //
				KuRuMiV3[setMap].set(commandName.toLowerCase(), command);
				commandLoadSuccess++;
