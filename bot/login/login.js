...
const login = defaultRequire("kurumi-fca");
...
const { callbackListenTime, storage5Message } = global.KuRuMiV3;
...
const titles = [
	...
	[
		"K U R U M i  V 3 @" + currentVersion
	],
	[
		"KURUMi V3"
	]
...
let subTitle = `KuRuMi-Bot V3@${currentVersion}- A powerful Facebook Messenger Bot by N1SA9`;
const subTitleArray = [];
...
const author = ("Created by N1SA9 with ♡");
const srcUrl = ("Source code: https://github.com/EpicDanger198/KuRuMiV3");
...
// const { config, configCommands } = global.KuRuMiV3;
const { facebookAccount } = global.KuRuMiV3.config;
...
async function getAppStateFromEmail(spin = { _start: () => { }, _stop: () => { } }, facebookAccount) {
	...
	global.KuRuMiV3.config.facebookAccount['2FASecret'] = code2FATemp || "";
	writeFileSync(global.client.dirConfig, JSON.stringify(global.KuRuMiV3.config, null, 2));
	return appState;
}
	...
	try {
		...
		// is cookie string
			...
			// is netscape cookie
				...
				global.KuRuMiV3.config.facebookAccount.email = splitAccountText[0]; // bug here=> email is "["
				global.KuRuMiV3.config.facebookAccount.password = splitAccountText[1]; // bug here=> password is "]"
...
function stopListening(keyListen) {
	...
	return new Promise((resolve) => {
		global.KuRuMiV3.fcaApi.stopListening?.(() => {
...
async function startBot(loginWithEmail) {
	...
	const tooOldVersion = (await axios.get("https://raw.githubusercontent.com/EpicDanger198/KuRuMi-V3-Storage/main/tooOldVersions.txt")).data || "0.0.0";
	// nếu version cũ hơn
	...
	(function loginBot(appState) {
		global.KuRuMiV3.commands = new Map();
		global.KuRuMiV3.eventCommands = new Map();
		global.KuRuMiV3.aliases = new Map();
		global.KuRuMiV3.onChat = [];
		global.KuRuMiV3.onEvent = [];
		global.KuRuMiV3.onReply = new Map();
		global.KuRuMiV3.onReaction = new Map();
		...
		login({ appState }, global.KuRuMiV3.config.optionsFca, async function (error, api) {
			...
			global.KuRuMiV3.fcaApi = api;
			global.KuRuMiV3.botID = api.getCurrentUserID();
			...
			log.info("PREFIX", global.KuRuMiV3.config.prefix);
			log.info("LANGUAGE", global.KuRuMiV3.config.language);
			log.info("BOT NICK NAME", global.KuRuMiV3.config.nickNameBot || "KuRuMi-V3");
			// ———————————————————— GBAN ————————————————————— //
			let dataGban;

			try {
				// convert to promise
				const item = await axios.get("https://raw.githubusercontent.com/EpicDanger198/KuRuMi-V3-Gban/master/gban.json");
				dataGban = {}//item.data;
				...
				// ———————————————— CHECK ADMIN ———————————————— //
				for (const idad of global.KuRuMiV3.config.adminBot) {
			...
			let notification;
			try {
				const getNoti = await axios.get("https://raw.githubusercontent.com/EpicDanger198/KuRuMi-V3-Gban/master/notification.txt");
			...
			if (global.KuRuMiV3.config.autoRefreshFbstate == true) {
				changeFbStateByCode = true;
				try {
			...
			if (global.KuRuMiV3.config.autoLoadScripts?.enable == true) {
				const ignoreCmds = global.KuRuMiV3.config.autoLoadScripts.ignoreCmds?.replace(/[ ,]+/g, ' ').trim().split(' ') || [];
				const ignoreEvents = global.KuRuMiV3.config.autoLoadScripts.ignoreEvents?.replace(/[ ,]+/g, ' ').trim().split(' ') || [];
					...
					if (filename.endsWith('.js')) {
						...
						if ((event == 'change' || event == 'rename') && existsSync(`${process.cwd()}/scripts/cmds/${filename}`)) {
							try {
								...
								const infoLoad = global.utils.loadScripts("cmds", filename, log, global.KuRuMiV3.configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData);
					...
					if (filename.endsWith('.js')) {
						...
						if ((event == 'change' || event == 'rename') && existsSync(`${process.cwd()}/scripts/events/${filename}`)) {
							try {
								...
								const infoLoad = global.utils.loadScripts("events", filename, log, global.KuRuMiV3.configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData);
								if (infoLoad.status == "success")
			...
			// ——————————————————— DASHBOARD ——————————————————— //
			if (global.KuRuMiV3.config.dashBoard?.enable == true && dashBoardIsRunning == false) {
			...
			let i = 0;
			const adminBot = global.KuRuMiV3.config.adminBot
			...
			log.master("LOAD TIME", `${convertTime(Date.now() - global.KuRuMiV3.startTime)}`);
			logColor("#f5ab00", createLine("COPYRIGHT"));
			// —————————————————— COPYRIGHT INFO —————————————————— //
			// console.log(`\x1b[1m\x1b[33mCOPYRIGHT:\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36mKuRuMi-V3 created by N1SA9 (https://github.com/N1SA9EDITZ), please do not sell this source code or claim it as your own. Thank you!\x1b[0m`);
			console.log(`\x1b[1m\x1b[33m${("COPYRIGHT:")}\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36m${("KuRuMiV3 created by N1SA9 (https://github.com/EpicDanger198), please do not sell this source code or claim it as your own. Thank you!")}\x1b[0m`);
			logColor("#f5ab00", character);
			global.KuRuMiV3.config.adminBot = adminBot;
			writeFileSync(global.client.dirConfig, JSON.stringify(global.KuRuMiV3.config, null, 2));
			writeFileSync(global.client.dirConfigCommands, JSON.stringify(global.KuRuMiV3.configCommands, null, 2));

			// ——————————————————————————————————————————————————— //
			const { restartListenMqtt } = global.KuRuMiV3.config;
			...
			async function callBackListen(error, event) {
				if (error) {
					...
					if (
						...
						if (global.KuRuMiV3.config.autoRestartWhenListenMqttError)
							process.exit(2);
						else {
							...
							if (intervalCheckLiveCookieAndRelogin == false) {
								...
								const interval = setInterval(async () => {
									...
									if (cookieIsLive) {
										...
										global.KuRuMiV3.Listening = api.listenMqtt(createCallBackListen(keyListen));
									}
								}, 5000);
							}
						}
				...
				global.responseUptimeCurrent = responseUptimeSuccess;
				global.statusAccountBot = 'good';
				const configLog = global.KuRuMiV3.config.logEvents;
				...
				if (
					global.KuRuMiV3.config.whiteListMode?.enable == true
					&& global.KuRuMiV3.config.whiteListModeThread?.enable == true
					// admin
					&& !global.KuRuMiV3.config.adminBot.includes(event.senderID)
				) {
					if (
						!global.KuRuMiV3.config.whiteListMode.whiteListIds.includes(event.senderID)
						&& !global.KuRuMiV3.config.whiteListModeThread.whiteListThreadIds.includes(event.threadID)
						// admin
						&& !global.KuRuMiV3.config.adminBot.includes(event.senderID)
					)
						return;
			...
			// ———————————————————— START BOT ———————————————————— //
			await stopListening();
			global.KuRuMiV3.Listening = api.listenMqtt(createCallBackListen());
			global.KuRuMiV3.callBackListen = callBackListen;
			// ——————————————————— UPTIME ——————————————————— //
			if (global.KuRuMiV3.config.serverUptime.enable == true && !global.KuRuMiV3.config.dashBoard?.enable && !global.serverUptimeRunning) {
				const http = require('http');
				const express = require('express');
				const app = express();
				const server = http.createServer(app);
				const { data: html } = await axios.get("https://raw.githubusercontent.com/EpicDanger198/resources-goat-bot/master/homepage/home.html");
				const PORT = global.KuRuMiV3.config.dashBoard?.port || (!isNaN(global.KuRuMiV3.config.serverUptime.port) && global.KuRuMiV3.config.serverUptime.port) || 3001;
				//app.get('/', (req, res) => res.send(html));
				app.get('/uptime', global.responseUptimeCurrent);
				let nameUpTime;
				try {
					if (global.KuRuMiV3.config.serverUptime.socket?.enable == true)
	...
	if (global.KuRuMiV3.config.autoReloginWhenChangeAccount) {
		setTimeout(function () {
...
global.KuRuMiV3.reLoginBot = startBot;
startBot();