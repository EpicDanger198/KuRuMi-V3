...
module.exports = async function (api, createLine) {
	...
	// ————————————————— AUTO SYNC ————————————————— //
	if (api && global.KuRuMiV3.config.database.autoSyncWhenStart == true) {
		...
		try {
			...
			for (const threadInfo of allThreadInfo) {
				...
				if (
					...
					const index = allThreadData.findIndex(
						thread => thread?.threadID === threadInfo.threadID
					);

					if (index !== -1) {
						allThreadData.splice(index, 1);
					}
			...
			api.setOptions({
				logLevel: global.KuRuMiV3.config.optionsFca.logLevel
			});
		}
	}
