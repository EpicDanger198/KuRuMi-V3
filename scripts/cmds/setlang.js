...
module.exports = {
	...
	onStart: async function ({ message, args, getLang, threadsData, role, event }) {
		...
		if (["-g", "-global", "all"].includes(args[1]?.toLowerCase())) {
			...
			global.KuRuMiV3.config.language = langCode;
			fs.writeFileSync(global.client.dirConfig, JSON.stringify(global.KuRuMiV3.config, null, 2));
			return message.reply(getLang("setLangForAll", langCode));
		}

		await threadsData.set(event.threadID, langCode, "data.lang");
		return message.reply((global.KuRuMiV3.commands.get("setlang")?.langs[langCode]?.setLangForCurrent || "Set default language for current chat: %1").replace("%1", langCode));
	}
};