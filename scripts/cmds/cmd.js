...
const { configCommands } = global.KuRuMiV3;
		...
		if (
			...
			if (fs.existsSync(path.join(__dirname, fileName)))
				return message.reply(getLang("alreadExist"), (err, info) => {
					global.KuRuMiV3.onReaction.set(info.messageID, {
						commandName,
...
function loadScripts(folder, fileName, log, configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode) {
	// global.KuRuMiV3[folderModules == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"].push({
	// 	filePath: pathCommand,
	// 	commandName: [commandName, ...validAliases]
	// });
	const storageCommandFilesPath = global.KuRuMiV3[folder == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"];
	...
	try {
		...
		const KuRuMiV3 = global["KuRuMiV3"];
		const { onFirstChat: allOnFirstChat, onChat: allOnChat, onEvent: allOnEvent, onAnyEvent: allOnAnyEvent } = KuRuMiV3;
		...
		// —————————————— CHECK COMMAND EXIST ——————————————— //
		if (!oldCommandName) {
			if (KuRuMiV3[setMap].get(oldCommandName)?.location != pathCommand)
				throw new Error(`${commandType} name "${oldCommandName}" is already exist in command "${removeHomeDir(KuRuMiV3[setMap].get(oldCommandName)?.location || "")}"`);
		}
		// ————————————————— CHECK ALIASES ————————————————— //
		...
		if (configCommand.aliases) {
			...
			for (const alias of aliases) {
				...
				if (KuRuMiV3.aliases.has(alias))
					throw new Error(`alias "${alias}" is already exist in ${commandType} "${KuRuMiV3.aliases.get(alias)}" with file name "${removeHomeDir(KuRuMiV3[setMap].get(KuRuMiV3.aliases.get(alias))?.location || "")}"`);
				KuRuMiV3.aliases.set(alias, scriptName);
			}
		}
		// ————————————————— CHECK ENVCONFIG ————————————————— //
		// env Global
		...
		KuRuMiV3[setMap].delete(oldCommandName);
		KuRuMiV3[setMap].set(scriptName, command);
...
function unloadScripts(folder, fileName, configCommands, getLang) {
	...
	const KuRuMiV3 = global["KuRuMiV3"];
	const { onChat: allOnChat, onEvent: allOnEvent, onAnyEvent: allOnAnyEvent } = KuRuMiV3;
	...
	delete require.cache[require.resolve(pathCommand)];
	KuRuMiV3[setMap].delete(commandName);
