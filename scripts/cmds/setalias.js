	...
	onStart: async function ({ message, event, args, threadsData, globalData, role, getLang }) {
		...
		switch (args[0]) {
			case "add": {
				...
				if (!global.KuRuMiV3.commands.has(commandName))
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						if (global.KuRuMiV3.aliases.has(alias))
							return message.reply(getLang("aliasExist", alias, global.KuRuMiV3.aliases.get(alias)));
						...
						global.KuRuMiV3.aliases.set(alias, commandName);
				...
				if (global.KuRuMiV3.commands.get(alias))
			...
			case "rm": {
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						await globalData.set('setalias', globalAliasesData, 'data');
						global.KuRuMiV3.aliases.delete(alias);
