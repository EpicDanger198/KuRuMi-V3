...
module.exports = {
	...
	langs: {
		vi: {
			noUpdates: "✅ | Bạn đang sử dụng phiên bản mới nhất của KuRuMi-Bot V3 (v%1).",
			updatePrompt: "💫 | Bạn đang sử dụng phiên bản %1. Hiện tại đã có phiên bản %2. Bạn có muốn cập nhật chatbot lên phiên bản mới nhất không?"
				...
				+ "\n%3%4"
				+ "\n\nℹ️ | Xem chi tiết tại https://github.com/N1SA9EDITZ/KuRuMiV3/commits/main"
		...
		en: {
			noUpdates: "✅ | You are using the latest version of KuRuMi-Bot V3 (v%1).",
			updatePrompt: "💫 | You are using version %1. There is a new version %2. Do you want to update the chatbot to the latest version?"
				...
				+ "\n%3%4"
				+ "\n\nℹ️ | See details at https://github.com/N1SA9EDITZ/KuRuMiV3/commits/main"
	...
	onStart: async function ({ message, getLang, commandName, event }) {
		// Check for updates
		const { data: { version } } = await axios.get("https://raw.githubusercontent.com/N1SA9EDITZ/KuRuMiV3/main/package.json");
		const { data: versions } = await axios.get("https://raw.githubusercontent.com/N1SA9EDITZ/KuRuMiV3/main/versions.json");
	...
	onReaction: async function ({ message, getLang, Reaction, event, commandName }) {
		...
		const { data: lastCommit } = await axios.get('https://api.github.com/repos/N1SA9EDITZ/KuRuMiV3/commits/main');
