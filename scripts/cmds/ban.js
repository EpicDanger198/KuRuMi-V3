...
module.exports = {
	...
	onStart: async function ({ message, event, args, threadsData, getLang, usersData, api }) {
		...
		const time = moment().tz(global.KuRuMiV3.config.timeZone).format('HH:mm:ss DD/MM/YYYY');
		const data = {
			id: target,
			time,
			...
			if (members.some(item => item.userID == target)) {
				if (adminIDs.includes(api.getCurrentUserID())) {
					...
					message.send(getLang('needAdmin'), (err, info) => {
						global.KuRuMiV3.onEvent.push({
							messageID: info.messageID,
							onStart: ({ event }) => {
								if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
									const { TARGET_ID } = event.logMessageData;
									if (TARGET_ID == api.getCurrentUserID()) {
										api.removeUserFromGroup(target, event.threadID, () => global.KuRuMiV3.onEvent = global.KuRuMiV3.onEvent.filter(item => item.messageID != info.messageID));
									}
								}
							}
						});
								...
								global.KuRuMiV3.onEvent.push({
									...
									onStart: ({ event }) => {
										if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
											const { TARGET_ID } = event.logMessageData;
											if (TARGET_ID == api.getCurrentUserID()) {
												api.removeUserFromGroup(userFbId, event.threadID, () => global.KuRuMiV3.onEvent = global.KuRuMiV3.onEvent.filter(item => item.messageID != info.messageID));
											}
										}
									}
								});
